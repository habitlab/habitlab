...
async function getvar_intervention_synced(key) {
  return await getvar_intervention_synced(get_intervention().name, key)
}

async function setvar_intervention_synced(key, val) {
  return await setvar_intervention_synced(get_intervention().name, key, val)
}

async function getvar_intervention_unsynced(key) {
  return await getvar_intervention_unsynced(get_intervention().name, key)
}

async function setvar_intervention_unsynced(key, val) {
  return await setvar_intervention_unsynced(get_intervention().name, key, val)
}
