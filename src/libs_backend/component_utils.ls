...
get_require_utils = memoizeSingleAsync ->>
  await SystemJS.import('libs_backend/require_utils')

get_list_requires = memoizeSingleAsync ->>
  await SystemJS.import('list_requires_multi')

list_html_imports_raw_paths = (html_text) ->>
  output = []
...
list_html_imports_as_jspm = (html_text) ->>
  output = []
...
export add_custom_component = (component_info) ->>
...
export get_custom_component_info = (component_name) ->>
  custom_components = localstorage_getjson('custom_components')
  if not custom_components?
    return
