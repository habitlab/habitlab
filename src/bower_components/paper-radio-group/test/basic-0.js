      ...
      suite('defaults', function() {
        ...
        test('group can have no selection', function (done) {
          var group = fixture('NoSelection');
          waitForIronItemsChanged(group, function() {
            expect(group.selected).to.not.be.ok;
        ...
        test('group can have a selection', function (done) {
          var group = fixture('WithSelection');
          waitForIronItemsChanged(group, function() {
            expect(group.selected).to.be.ok;
            var items = group.items;
            expect(items.length).to.be.equal(3);
            ...
            expect(items[1].checked).to.be.equal(false);
            expect(items[2].checked).to.be.equal(false);
            expect(items[0].getAttribute('name')).to.be.equal(group.selected);
